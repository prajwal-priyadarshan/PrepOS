...
import { createNote, emptyState } from '../src/lib/model';
...
describe('createNote', () => {
  ...
  it('stamps the study day and the absolute time from one instant', () => {
    const note = createNote({ body: 'ratio trick', section: 'QA', at });

    expect(note.studyDay).toBe('2026-08-28');
    expect(note.createdAt).toBe(at.toISOString());
    expect(note.id).not.toBe(createNote({ body: 'x', section: 'QA', at }).id);
  });

  it('counts a note written after midnight toward the day it belongs to', () => {
    const lateNight = new Date(2026, 7, 29, 1, 30, 0);

    expect(createNote({ body: 'still today', section: 'QA', at: lateNight }).studyDay).toBe(
      '2026-08-28',
    );
  });

  it('trims the body', () => {
    expect(createNote({ body: '  spaced  \n', section: 'GENERAL', at }).body).toBe('spaced');
  });

  it('omits context rather than storing empty keys', () => {
    const bare = createNote({ body: 'no file open', section: 'GENERAL', at });
...
describe('notes in persisted state', () => {
  it('survives a write and read', () => {
    const state = {
      ...emptyState(),
      notes: [createNote({ body: 'keep me', section: 'QA', at: new Date(2026, 7, 28, 9) })],
    };
